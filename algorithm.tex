...
\begin{lstlisting}[caption=后缀数组]
// 因为倍增时可能访问越界数组，所以要开 2 倍大小
// n 为后缀个位，m 为桶的个数
int n, m;
// x 是桶数组，记录每个元素属于哪个桶；y 是辅助数组；c 是计数数组，记录每个桶中元素个数
...
void get_sa() {
	...
	for(int k = 1; k <= n; k <<= 1) {
		...
		// 将后缀放入桶数组中，即重新维护利用当前看到的关键字推断出的桶
		for(int i = 1; i <= n; i++)
			y[i] = x[i];
        m = 0;
		for(int i = 1; i <= n; i++)
